...
def roundNumbers(prices, target):
    ...
    def find_paths(idx=0,total=0,error=0,path=[]):
        ...
        #if total > target or (total == target and len(path) < len(prices)):
        #    print("-")
        #    return False
        # Reached the last price
        # Evaluate if path reached the target
        if idx >= len(prices):
            if sum(path) == target:
        ...
        for rounded in [price_down, price_up]:
            ...
            if memo_code not in memo:
                ...
                find_paths(
                    ...
                    path=p)
                memo.add(memo_code)
            #memo_rounded["{}:{}".format(idx,rounded)] = (total, error, path)

    # Find paths
    find_paths()
    print(sorted(list(memo)))
    # No paths to target were found...
    if len(target_combos) <= 0:
        return False
    # Sort by minimal error
    target_combos = sorted(target_combos, key= lambda e: (e[1]))
    # Return the path with minimum error
    print(target_combos)
